...
export function ChatInterface({ className, showUpgradePrompt = false }: ChatInterfaceProps) {
          ...
          <div key={msg.id} className={cn("flex gap-4", msg.role === "user" ? "justify-end" : "justify-start")}>
            ...
            <div className={cn("max-w-[75%]", msg.role === "user" ? "order-first" : "")}>
              <Card
                className={cn(
                  "p-4 shadow-sm",
                  msg.role === "user" ? "bg-primary text-primary-foreground ml-auto" : "bg-card border-border",
                )}
              >
        ...
        <div className="space-y-3">
          <div className="relative">
            <Textarea
              ...
              className="min-h-[100px] pr-24 resize-none text-sm"
              onKeyDown={(e) => {
